...
- `standalone/` — a single, offline, no-server browser build of the converter
  (`statement_converter.html`). It is generated by `python standalone/build.py`,
  which inlines `parser.js` (a JS port of the tool's text parsing/verification) and
  base64-embeds the vendored pdf.js. Rebuild after editing `parser.js`,
  `app.template.html`, or `vendor/`; `parser.js` is unit-testable with Node.
